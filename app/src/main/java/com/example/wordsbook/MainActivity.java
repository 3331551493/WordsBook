...
public class MainActivity extends AppCompatActivity {
    TextView textView;
    private CreatDatabase dbHelper = new CreatDatabase(this, "Ven.db", null, 1);//连接数据库
    ...
    public void queryALL() {
        initWords();//初始化单词列表
        //在listview里面显示单词列表
        WordsAdapter adapter = new WordsAdapter(MainActivity.this, R.layout.words, wordsList);
        ...
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                if (flag == 1) {//横屏才操作，显示释义及例句
                    ...
                    textView = (TextView)findViewById(R.id.land_text);
                    textView.setText("");
                    textView.setText("");
                    textView.append("中文：\n" + meanings);
                    textView.append("\n\n");
    ...
    public void addWord() {
        ...
        //加载事先编好的界面dialog.xml
        final View textEntryView = factory.inflate(R.layout.log, null);
        //三个输入框
            ...
            public void onClick(DialogInterface dialog, int i) {
                try {
                    ...
                    if (cursor.moveToFirst()) {
                        ...
                        Toast.makeText(getApplicationContext(), "单词"+s1 +"删除成功", Toast.LENGTH_LONG).show();
                    } else {
                        Toast.makeText(getApplicationContext(), "单词不存在", Toast.LENGTH_LONG).show();
                    }
                    cursor.close();
                } catch (Exception e) {
                    Toast.makeText(getApplicationContext(), "删除失败,请检查", Toast.LENGTH_LONG).show();
                }
                queryALL();
            }
            ...
            public void onClick(DialogInterface dialog, int i) {
                try {
                    ...
                    if (cursor.moveToFirst()) {//如果存在
                        ...
                        String ins = "insert into WordsBook values (?,?,?)";
                        db.execSQL(ins, new String[]{s1, s2, s3});
                        Toast.makeText(getApplicationContext(), "单词" + s1 +"更新成功", Toast.LENGTH_LONG).show();
                    } else {
                        Toast.makeText(getApplicationContext(), "单词不存在", Toast.LENGTH_LONG).show();
                    }
                    cursor.close();
